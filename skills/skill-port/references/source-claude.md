...
| `.mcp.json` or MCP server configs | Treat as dependency-bound but mappable. Capture server names, endpoint type/URL, auth requirements, likely tool prefixes, and target setup/config snippets with placeholders. |
...
- If a source contains plugin metadata and skills, prefer grouped target skill bundles plus metadata notes over a flat skill-only copy when plugin behavior affects discovery, commands, agents, or dependencies.
